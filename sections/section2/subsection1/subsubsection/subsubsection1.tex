...
\hspace*{2em} Hệ thống được thiết kế theo mô hình \textit{client–server} ba lớp, trong đó mỗi thành phần đảm nhận vai trò riêng nhằm bảo đảm hiệu năng và tính bảo mật trong quá trình truyền tải thông tin.

\indent \textbf{Lớp Client:} Ứng dụng được phát triển bằng \textit{Flutter} (ngôn ngữ Dart), chịu trách nhiệm hiển thị giao diện trò chuyện, quản lý danh sách hội thoại, gửi và nhận tin nhắn, cùng các thao tác tải lên hoặc tải xuống dữ liệu.  
Tại lớp này, mọi hoạt động mã hóa và giải mã tin nhắn đều được xử lý hoàn toàn cục bộ nhằm đảm bảo nguyên tắc mã hóa đầu cuối (E2EE), qua đó ngăn máy chủ hoặc tin tặc truy cập vào nội dung tin nhắn.
...
Mọi phiên client khác cùng tham gia hội thoại đó sẽ nhận được các sự kiện như tin nhắn mới hoặc lời mời kết bạn.

\indent \textbf{Lớp Server:} Máy chủ được triển khai bằng \textit{Node.js}, cung cấp \textit{REST API} và \textit{WebSocket Gateway}, đảm nhận vai trò trung gian trong việc truyền tải dữ liệu giữa các người dùng, xử lý xác thực, lưu trữ thông tin và các yêu cầu nghiệp vụ.  
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.85\textwidth]{figures/pic7.jpg}
  \caption{Mô hình ba lớp Client–Server–Database của hệ thống.}
  \label{fig:pic7}
\end{figure}


% Bảng conversation
\hspace*{2em} Bảng ~\ref{tab:conversation} mô tả chi tiết các trường dữ liệu của \textbf{bảng Conversation}, được sử dụng để lưu trữ thông tin về các cuộc trò chuyện trong hệ thống.  
...
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động của bản ghi. \\ \hline
\end{tabularx}
\end{table}


% Bảng friendship
\hspace*{2em} Bảng ~\ref{tab:friendship} mô tả \textbf{cấu trúc bảng dữ liệu Friendship}, được dùng để quản lý mối quan hệ bạn bè giữa các người dùng trong hệ thống.  
...
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động. \\ \hline
\end{tabularx}
\end{table}

% Bảng membership
\hspace*{2em} Bảng ~\ref{tab:membership} mô tả \textbf{cấu trúc bảng dữ liệu Membership}, được sử dụng để quản lý thông tin thành viên trong mỗi cuộc trò chuyện.  
...
\texttt{lastReadAt} & Date & Thời điểm người dùng đọc tin nhắn cuối cùng. \\ \hline
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động. \\ \hline
\end{tabularx}
\end{table}

% Bảng message
\hspace*{2em} Bảng ~\ref{tab:message} trình bày \textbf{cấu trúc bảng dữ liệu Message}, nơi lưu trữ toàn bộ thông tin liên quan đến các tin nhắn trong cuộc trò chuyện.  
Mỗi bản ghi biểu thị một tin nhắn được gửi bởi người dùng, bao gồm nội dung văn bản, tệp đính kèm và các mốc thời gian tương ứng.
...
\texttt{media} & Object & Thông tin tệp phương tiện: URL, MIME type, kích thước. \\ \hline
\texttt{captions} & String & Chú thích cho hình ảnh hoặc tệp đính kèm. \\ \hline
\texttt{timestamp} & Date & Thời gian gửi tin nhắn. \\ \hline
\texttt{editedAt} & Date & Thời điểm chỉnh sửa tin nhắn (nếu có). \\ \hline
\texttt{deletedAt} & Date & Thời điểm xóa tin nhắn. \\ \hline
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động của bản ghi. \\ \hline
\end{tabularx}
\end{table}

% Bảng messagekey
\hspace*{2em} Bảng ~\ref{tab:messagekey} mô tả \textbf{cấu trúc bảng dữ liệu MessageKey}, được sử dụng để lưu trữ khóa mã hóa AES của từng tin nhắn.  
Mỗi bản ghi thể hiện một khóa AES được mã hóa bằng RSA, gắn liền với người dùng sở hữu khóa và tin nhắn tương ứng.
...
\hline
\textbf{Trường} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} \\ \hline
\texttt{messageId} & ObjectId (Message) & Tin nhắn tương ứng chứa khóa AES. \\ \hline
\texttt{userId} & ObjectId (User) & Người dùng sở hữu khóa AES đã mã hóa. \\ \hline
\texttt{encryptedAesKey} & String & Khóa AES được mã hóa bằng thuật toán RSA. \\ \hline
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động của bản ghi. \\ \hline
\end{tabularx}
\end{table}

% Bảng user
\hspace*{2em} Bảng ~\ref{tab:user} mô tả \textbf{cấu trúc bảng dữ liệu User}, được sử dụng để lưu trữ thông tin người dùng trong hệ thống.  
...
\texttt{publicKey} & String & Khóa công khai RSA được sử dụng để mã hóa tin nhắn gửi đến người dùng. \\ \hline
\texttt{timestamps} & Date & Thời gian tạo và cập nhật tự động của bản ghi. \\ \hline
\end{tabularx}
\end{table}

\hspace*{2em} Tổng thể, hệ thống cơ sở dữ liệu được thiết kế nhằm đảm bảo khả năng mở rộng, truy xuất hiệu quả và bảo mật thông tin người dùng.  
Các bảng dữ liệu được liên kết chặt chẽ với nhau: \textbf{User} quản lý danh tính và khóa công khai RSA, \textbf{Conversation} và \textbf{Membership} xác định cấu trúc các cuộc trò chuyện, trong khi \textbf{Message} và \textbf{MessageKey} lưu trữ nội dung tin nhắn cùng các khóa AES đã được mã hóa.  
